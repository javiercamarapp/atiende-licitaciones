 ...
 *
 * El README (§"Arquitectura del monorepo") describe `packages/sources`
 * como "librería pura: conectores + matching, determinista + versiones,
 * sin DB, sin red propia en tests" -- consumida por `apps/worker`, nunca
 * al revés. Su `package.json` ya refleja esto: solo declara `zod` como
 * dependencia de runtime, ningún framework HTTP/UI. Pero nada verificaba
 * esa frontera de forma automática: si alguien agregara mañana un
 * `import` de `fastify`/`express` (acoplando el dominio al servidor
 * HTTP) o de `react`/`vite` (acoplándolo a la UI), o una ruta relativa
 * hacia `apps/*` (invirtiendo la dependencia: el dominio no debe conocer
 * a sus consumidores), nada lo detectaba hasta notarse en runtime o
 * revisión manual.
 *
...
describe("Patrón #2: frontera de capas dominio vs framework (@atiende/sources)", () => {
        ...
        if (match) {
          ...
          throw new Error(
            `${relative(SRC_ROOT, filePath)}:${line} — ${pattern.description} ("${match[0]}"). ` +
              "packages/sources es una librería pura consumida por apps/worker, nunca al revés.",
          );
        }
      }
    },
