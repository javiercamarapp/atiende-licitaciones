 ...
 *
 * El README (§"Arquitectura del monorepo") describe `packages/expediente`
 * como "librería pura" -- requisitos, datos de empresa, propuesta
 * técnica/económica, checklist, paquete final -- consumida por `apps/api`,
 * nunca al revés. Su `package.json` ya refleja esto en la práctica: solo
 * declara `jszip` como dependencia de runtime, ningún framework HTTP/UI.
 * Pero nada verificaba esa frontera de forma automática: si alguien
 * agregara mañana un `import` de `fastify`/`express` (acoplando el
 * dominio al servidor HTTP) o de `react`/`vite` (acoplándolo a la UI), o
 * una ruta relativa hacia `apps/*` (invirtiendo la dependencia: el
 * dominio no debe conocer a sus consumidores), nada lo detectaba hasta
...
// Cubre `import ... from "x"`, `import("x")` (dinámico) y `require("x")`,
// tanto para el nombre exacto del paquete como para un subpath
// (`"fastify/plugin"`, `"react-dom/client"`).
...
describe("Patrón #2: frontera de capas dominio vs framework (@atiende/expediente)", () => {
        ...
        if (match) {
          ...
          throw new Error(
            `${relative(SRC_ROOT, filePath)}:${line} — ${pattern.description} ("${match[0]}"). ` +
              "packages/expediente es una librería pura consumida por apps/api, nunca al revés.",
          );
        }
      }
    },
