...
| `manifest-template.ts` | `ManifestTemplateRegistry` (REQ-028): motor real de plantillas versionadas de **manifiestos "bajo protesta"** (declaraciones legales firmadas por el representante, distintas del `PackageManifest`/`manifiesto.json` de `package-assembler.ts`, que es el índice del ZIP). Cada plantilla es un `manifest.json` real: `legalBasis` no vacía, `variables` tipadas (string/number/boolean/date/enum), `body` con placeholders `{{var}}` validados 1:1 contra `variables` al registrar, `version` semver estrictamente creciente por `id` (migración de versión — `registerVersion` rechaza duplicar o retroceder), `sha256` recalculado siempre por el registro (nunca aceptado como input). Toda versión nace `reviewStatus: "needs_human"` (gate `legal-doc`) — solo `approveVersion(id, version, approvedBy, approvedAt)` la promueve a `"approved"`; `render()` es fail-closed: bloquea (`ok:false`, acumulando TODOS los huecos/tipos incorrectos, nunca lanza ni sustituye por texto inventado) si la plantilla no existe, no está aprobada, o falta/no tipa una variable. Éxito trae `docId: {templateId, templateVersion, templateSha256}` — el "doc_id de respaldo" del criterio de aceptación. Ninguna plantilla de PRODUCCIÓN está registrada todavía: eso exige redacción/aprobación legal humana real (mismo bloqueo de cita de artículo vigente que REQ-117/BLOQUEOS.md); el motor es real y completo alrededor de ese hueco explícito. | REQ-028 |
