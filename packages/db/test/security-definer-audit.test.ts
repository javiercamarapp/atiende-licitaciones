...
const SECURITY_DEFINER_WHITELIST: Record<string, string> = {
  ...
  'app.record_auth_event(text,uuid,jsonb,text)':
    'API-13 (docs/auditoria-1/db-api-seguridad-reverificacion.md, 0051_fix_api13_auth_audit_log.sql): no acepta ningún identificador de ORGANIZACIÓN (nunca resuelve org_id de un parámetro externo -- siempre inserta org_id=null en audit_log, bypassing RLS SOLO para eso). El p_actor_id que recibe es un actor_id de auditoría (metadato del evento, no una credencial ni un control de acceso) que el propio llamador (apps/api/src/modules/auth/routes.ts) ya resolvió y verificó ANTES de invocarla (login: contraseña recién validada o null si el email no existe; refresh: sub/jti de un JWT firmado por el servidor o el user_id de la fila de refresh_tokens ya revocada que causó el reuso; logout: sub de un JWT válido) -- no hay ningún control de acceso que este parámetro pueda eludir, a diferencia de DB-01/DB-08/DB-12 (donde el parámetro elegía DE QUIÉN mutar/leer datos). Además, `p_action` está restringido en SQL a una lista fija de 5 acciones de autenticación (raise exception si no coincide), así que tampoco puede usarse para escribir un action/entity arbitrario en audit_log.',
};
