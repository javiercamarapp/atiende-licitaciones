...
const SECURITY_DEFINER_WHITELIST: Record<string, string> = {
  ...
  'app.record_access_denied_event(uuid,uuid,text,text,text,jsonb)':
    'Patrón Likida/atiende.ai #6 (0099_patron6_access_denied_audit.sql), mismo patrón que app.record_auth_event/app.record_security_event: registra un intento de acceso denegado por rol (`plugins/error-handler.ts`, todo AppError con statusCode 403) para CUALQUIER p_org_id, incluido NULL -- necesario porque el actor denegado casi siempre NO cumple la política RLS normal de audit_log (no es miembro de esa organización, o no es superadmin), que es justo la razón de la denegación. `action` queda fija en el cuerpo de la función ("access.denied", nunca un parámetro), y exige SIEMPRE que p_actor_id coincida con app.current_user_id() ya fijado por el llamador (mismo criterio "actor_no_coincide" que record_security_event) -- ningún llamador con app_role puede atribuir el evento a un actor_id arbitrario.',
  ...
  // --- REQ-060 (0103_req060_oic_module.sql): módulo de lado comprador (OIC/contraloría) ---
  ...
  // --- REQ-026/REQ-111/REQ-112 (0107_req026_kyc_negativo_69b_fingerprint.sql): KYC negativo 69-B y fingerprint de interpósita persona ---
