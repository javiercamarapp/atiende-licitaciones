...
| EX-EXP-06 (PARCIAL) / EX-EXP-15 | **Corregido** | `fix(expediente): EX-EXP-06/EX-EXP-15 fecha numérica ambigua baja confidence`. `extractDeadline` (`src/requirement-matrix.ts`) marca `ambiguousDate` cuando día y mes son ambos ≤12 en el patrón numérico (p. ej. "05/09/2026"); `classifySentence` baja `confidence` a 0.5 en ese caso, sin cambiar la interpretación DD/MM por defecto. Test: `test/requirement-matrix.test.ts` describe "EX-EXP-15"; se verificó en worktree separado que 1/363 tests fallaba contra el código previo. Nota de trazabilidad: por una condición de carrera del índice de git compartido entre agentes concurrentes en el árbol principal, este cambio (código+tests+docs) quedó commiteado dentro del commit `587d4a2` (ajeno, de `packages/agents`) en vez de en un commit propio; contenido verificado íntegro y correcto vía `git diff`/`git show` tras el hecho. |
| EX-EXP-16 | **Corregido (documentación)** | `docs(expediente): EX-EXP-16 docstring de expediente-flow.test.ts no sobreestima A6-A15`. Se reformuló el docstring de `test/expediente-flow.test.ts` para dejar explícito que el test cubre el flujo integrado *dentro de la librería pura* (precondición necesaria, no suficiente) y que A6-A15 siguen PENDIENTE en `docs/ACEPTACION.md` hasta que exista integración real API/UI. Sin cambio de comportamiento ni de aserciones — no aplica "test que falla" (hallazgo puramente documental). |
