...
> "Estado reparación" se agregó en la micro-vuelta posterior a esta
> reverificación (corrector Sonnet, dos hallazgos, ver
> `docs/logs/fix-sources-micro.log`) para dar seguimiento a SR-23/SR-24, el
> mismo patrón que ya usan `sources-cierre.md` §4 y las demás
> `*-cierre*.md`/`*-reverificacion*.md` de `docs/auditoria-1/`.

| ID | Severidad | Relacionado con | Hallazgo (evidencia) | Estado reparación (micro-vuelta, `docs/logs/fix-sources-micro.log`) |
|---|---|---|---|---|
| SR-23 | MEDIA | SR-20 | `assertLegitimateResponseBody` con `minimalContentMarkers` (DOF) tiene doble filo: (a) **falso positivo** confirmado — una nota DOF con contenido de negocio real pero plantilla ligeramente distinta de los 2 regex hardcodeados se rechaza como `interface_changed` (el formato real de una nota con convocatorias nunca fue confirmado en vivo, según el propio README); (b) **falso negativo** confirmado end-to-end con `DiscoveryPipeline` real — un interstitial JS genérico ("verificando su navegador...") que preserva el `<title>`/`id` estáticos del sitio real de DOF pasa sin lanzar, `health.state="ok"`, `coverage.emptyResult=true` — la misma violación de REQ-148 que SR-20 debía cerrar, vía un vector (bloqueo que reutiliza el shell HTML del sitio) no contemplado por el marcador estructural mínimo actual. | **Corregido.** `assertLegitimateResponseBody` (`src/http/response-classifier.ts`) reemplaza `minimalContentMarkers` (plantilla fija) por `semanticContentMarkers` (contenido semántico: "convocatoria"/"licitación pública"/patrón de número de procedimiento, configurable por fuente) + `detectInterstitialShellMarker` (meta-refresh, `<script>` con `setTimeout`/`location.replace`/`location.href`/`window.location`, o cuerpo con menos de `minUsefulTextBytes` -- default 120 -- de texto útil tras quitar tags/scripts/estilos). DOF (`dof-connector.ts`) usa `DOF_SEMANTIC_CONTENT_MARKERS`. Tests: `test/response-classifier.test.ts` (plantilla distinta no lanza; script-redirect/meta-refresh/cuerpo insuficiente sí lanzan `InterfaceChangedError`; un vendor conocido sigue clasificando `CaptchaDetectedError`) y `test/connectors/dof.test.ts` §"SR-23" (fixtures `nota-plantilla-distinta.html` -> `ok`, `interstitial-cascaron.html` -> `interface_changed` vía `DiscoveryPipeline` real, captcha real -> `captcha_detected`, regresión). |
| SR-24 | **ALTA** | SR-21 | `ctx.reportDropped` solo está conectado en `compras-mx-connector.ts`/`compras-mx-historical-csv-connector.ts`. `create-ocds-connector.ts` (compartido por `OcdsShcpConnector`, `PdnS6Connector`, `StatePortalConnector` — 3 de los 4 `SourceId` no-ComprasMX) nunca lo invoca; `ocds-mapper.ts` descarta releases sin bloque `tender` con un `return null` silencioso. Confirmado end-to-end: un release package con 80% de sus releases sin `tender` produce `health.state="ok"`, `dropped:[]`, `errores:[]`, `coverage.emptyResult:false` — la MISMA violación de REQ-148 que motivó SR-21, invisible para 3 de las 4 fuentes que no son ComprasMX. La invariante "ningún registro se pierde en silencio" que la tabla de `sources-cierre.md` §4/§9 y el README (sección "Salud explícita por fuente") describen como ya cerrada solo es cierta para ComprasMX. | **Corregido.** `mapOcdsPackageToTenderRecords` (`ocds-mapper.ts`) ahora devuelve `{records, dropped}` (mismo contrato que `mapComprasMxApiRecords`, SR-21): cada release valida individualmente con `OcdsReleaseSchema.safeParse` (release inválido -> `dropped[]` con el motivo del `ZodError`, sin tumbar el release package completo) y cada release sin `tender` también se reporta en `dropped[]` (sigue filtrándose, ya no en silencio). `create-ocds-connector.ts` y `state-portal-connector.ts` reenvían cada entrada a `ctx.reportDropped`, igual que ya hacía `compras-mx-connector.ts` -- `DiscoveryPipeline.dropRateThreshold` (ya genérico, sin cambios) protege ahora a los 5 conectores registrados, no solo ComprasMX. DOF (`dof-mapper.ts`/`dof-connector.ts`) recibe el mismo patrón por generalización (`mapDofNoticeToTenderRecordSafe`, `extractDofNoticesFromText` devuelve `{notices, dropped}`), aunque sus fallbacks de regex hacen improbable alcanzar un aviso inválido con HTML real (documentado en el propio código; probado con `dof-mapper.ts` mockeado). Tests: `test/connectors/ocds.test.ts` §"SR-24" (release sin `ocid` -> `dropped`; release package con 80% sin `tender` vía `DiscoveryPipeline` real -> `interface_changed`, replica exactamente el vector de esta reverificación), `test/schema-null-normalization.test.ts` (actualizado), `test/connectors/dof.test.ts` §"SR-24", y el test de INVARIANTE `test/connectors/drop-invariant.test.ts` que recorre los 5 conectores del `ConnectorRegistry` con un fixture de 1 registro/release inválido cada uno y confirma `dropped.length >= 1` + `errores[]` con motivo + reclasificación `interface_changed` (>20%) en TODOS, no solo ComprasMX. |
