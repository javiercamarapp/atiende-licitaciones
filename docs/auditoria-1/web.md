...
|---|---|---|---|---|---|
| W-01 | **Crítica** | 1, 8 | `packages/agents`, `packages/sources`, `packages/db`, `packages/expediente` y `apps/api` existen en disco pero **nunca fueron añadidos a git** en ningún commit (`git log --all -- packages/` vacío; un clon limpio de `a43efd7` solo trae `apps/web/`, `docs/`, `package.json`), pese a que varios commits/documentos narran esos módulos como completados | Añadir esos directorios a git con `git add`/commit explícito, o si se decide mantenerlos fuera del repo principal, documentarlo y dejar de reportarlos como "despachados" en `docs/PROGRESO.md`/`docs/DECISIONES.md` | **Resuelto por otros agentes** (commits `c56e20f`, `8052ff4`, `467be32`, posteriores a esta auditoría) — `packages/*` y `apps/api` ya están en git. Fuera del ámbito exclusivo de este agente (`apps/web`); no se generó commit propio para este hallazgo. Verificado en el clon limpio usado para W-02/W-04 (`docs/logs/fix-web-w02.log`, `docs/logs/fix-web-w04.log`): el árbol ya trae `packages/*` y `apps/api`. |
| W-02 | **Crítica** | 1 | `npm run -w apps/web typecheck` y `build` **fallan** en un clon limpio (`TS2307`/`TS2339` en `vite.config.ts`) porque `apps/web/package.json` nunca declara `@types/node`; solo "funcionan" hoy porque `@types/node` queda hospedado en la raíz gracias a W-01 | Agregar `"@types/node"` (versión fija) a `devDependencies` de `apps/web/package.json` | **Corregido.** Commit `a9e45c2`. `apps/web/package.json` declara `"@types/node": "^22.10.2"` explícitamente. Reproducido el fallo real en una copia aislada de `apps/web` sin la dependencia y confirmado el fix en la misma copia (`docs/logs/fix-web-w02.log`); reverificado en un clon de git verdaderamente limpio al final de la ronda (`docs/logs/fix-web-ronda1.log`, sección "VERIFICACION EN CLON LIMPIO"): typecheck/lint/test/build/test:e2e en verde. |
| W-03 | Alta | 1 | No hay `package-lock.json` versionado en git (ni raíz ni `apps/web`) en ningún commit; `npm install` resuelve rangos semver frescos en cada corrida | Commitear el lockfile generado (o migrar los scripts a `npm ci`) | **Bloqueado, fuera de ámbito.** `package-lock.json` es un único archivo de todo el monorepo (workspaces de npm), no de `apps/web` exclusivamente; committearlo incluiría las dependencias resueltas de `apps/api`/`packages/*`, que otros agentes tienen en curso con cambios sin commitear. El ámbito de este agente (`apps/web/**`, `docs/logs/fix-web-*.log`, la columna de esta tabla) no incluye el lockfile raíz. Requiere coordinación a nivel de repositorio completo — no se generó commit propio. |
| W-04 | Alta | 1, 6 | El reporte "5 vulnerabilities" de `docs/logs/web-ronda1.log` no pertenece a `apps/web`: en clon limpio `npm audit --workspace apps/web` da 0; en el repo de trabajo el mismo comando sigue reportando 5 por una copia anidada vieja de vite/vitest ligada a W-01 — el filtro `--workspace` de `npm audit` no aísla el árbol real | Resolver W-01/W-03 y re-auditar `apps/web` de forma aislada; no confiar en `npm audit --workspace` cuando hay hoisting cruzado entre workspaces no versionados | **Corregido (documentación).** Commit `2f85346`. Con W-01 ya resuelto por otros agentes, `npm audit --workspace apps/web` vuelve a reportar "5 vulnerabilities" **incluso en un clon limpio** — pero ahora se debe a que `apps/api`/`packages/*` fijan `vitest@^2.1.8` (con `esbuild@0.21.5` vulnerable vía `vite@5.4.21`), no a `apps/web` (que usa `vitest@4.1.11`/`esbuild@0.25.12`, sin vulnerabilidades). Verificado con `npm ls vitest`/`npm ls esbuild --all` (`docs/logs/fix-web-w04.log`) y documentado en `apps/web/README.md` § "Seguridad de dependencias". No se puede alcanzar 0 vulnerabilidades reales sin que los workspaces fuera de ámbito suban de versión. |
| W-05 | Alta | 2, 3 | Violación **serious** de `color-contrast` (2.49:1, requerido 3:1) en modo oscuro: `AtiendeWordmark` (`AtiendeLogo.tsx:51`) usa `style={{ color: "#1D4ED8" }}` fijo, copiado tal cual del componente de referencia, que no se adapta al fondo oscuro | Sustituir el color inline por un token Tailwind (`text-primary` o equivalente) con valor propio en `.dark` | **Corregido.** Commit `435ecbc`. `AtiendeWordmark` usa la clase `text-primary` en vez del `style` inline. Verificado con axe-core en Chromium real (no jsdom) antes/después del fix: `docs/logs/fix-web-w05-w06.log`; test de regresión `apps/web/e2e/contraste.spec.ts`. |
| W-06 | Alta | 3, 5 | Violación **serious** de `color-contrast` (3.28:1, requerido 4.5:1) en modo claro sobre el badge `warning` de `SourceStatusBadge` (estados "CAPTCHA" y "Cambio de interfaz" — precisamente las dos alertas de fuente caída) | Oscurecer el token `--warning` en modo claro o usar texto oscuro sobre el fondo actual; re-medir con axe | **Corregido.** Commit `435ecbc` (mismo commit que W-05). `--warning-foreground` en modo claro pasa de blanco a un tono oscuro (~5.2:1 sobre `--warning`). Verificado con axe-core en Chromium real antes/después: `docs/logs/fix-web-w05-w06.log`; test de regresión `apps/web/e2e/contraste.spec.ts`. |
| W-07 | Media | 3 | Salto de encabezado h1→h3 (sin h2): `CardTitle` (`card.tsx:21`) está fijo a `<h3>` sin forma de configurar el nivel; se repite en cualquier página que combine `SectionHeader` + `Card` | Añadir prop de nivel de encabezado a `CardTitle` (o usar `<h2>` en `SectionHeader`) y ajustar los usos afectados | **Corregido.** Commit `9b5f43a`. `CardTitle` acepta una prop `level` (1-6, por defecto 2). Reproducido el salto de encabezado con axe-core real antes del fix y verificado en verde después (`docs/logs/fix-web-w07.log`); tests `apps/web/src/components/ui/card.test.tsx` y `apps/web/e2e/heading-order.spec.ts`. |
| W-08 | Media | 3 | `LoginPage` no tiene landmark `<main>` ni `<h1>` ni envuelve su contenido en un landmark (3 violaciones *moderate* de axe); es la única pantalla que no usa `AppShell` | Envolver el formulario en `<main>` y usar un `<h1>` real para el título | **Corregido.** Commit `0151404` (junto con W-09, mismas líneas de `LoginPage.tsx`). `LoginPage` ahora es un `<main>` con un `<h1>` real. Reproducido con axe-core real antes del fix y verificado en verde después (`docs/logs/fix-web-w08-w09.log`); test `apps/web/src/pages/LoginPage.test.tsx` (jsdom) y `apps/web/e2e/login-landmarks.spec.ts` (navegador real). |
| W-09 | Alta | 3 | El skip-link de `LoginPage` (`#login-form`) no mueve el foco al activarse (el objetivo es un `<div>` sin `tabindex`) — verificado por teclado real, no detectado por axe automatizado | Dar `tabIndex={-1}` al contenedor `#login-form` o redirigir el `id`/foco a un elemento real | **Corregido.** Commit `0151404` (junto con W-08). Se añade `tabIndex={-1}` al contenedor de `Tabs` y un componente `<SkipLink/>` compartido que llama `.focus()` explícitamente (usado también en `AppShell`). Reproducido el fallo por teclado real (Tab + Enter deja el foco en `<body>`) antes del fix y verificado en verde después (`docs/logs/fix-web-w08-w09.log`); test `apps/web/e2e/skip-link.spec.ts`. |
| W-10 | Baja | 4 | Botón hamburguesa mide 40×40px y el primer link del drawer móvil 36px de alto, por debajo de ≥44×44px recomendado | Aumentar tamaño/padding de esos elementos en el contexto móvil | **Corregido.** Commit `533fbf3`. Botón hamburguesa a `h-11 w-11` (44px); links de `SidebarNav` de `py-2` a `py-3` (44px de alto a `text-sm`). Reproducido con `boundingBox()` real a 390×844 antes del fix y verificado en verde después (`docs/logs/fix-web-w10.log`); test `apps/web/e2e/touch-targets.spec.ts`. |
| W-11 | Alta | 2 | `docs/investigacion/frontend-restaurantes.md` describe el login real de forma incompleta (sin Google OAuth, sin hero, sin Fraunces/Instrument Sans); `LoginPage.tsx` de licitaciones, fiel a ese documento, no se parece al login real de la marca | Corregir/ampliar el documento de investigación con la anatomía real; decidir explícitamente si licitaciones adopta esa misma anatomía o documenta la divergencia como decisión de producto | **Corregido dentro del ámbito de `apps/web`.** Commit `3ab333c`. `LoginPage.tsx` se rediseña a pantalla partida con kicker, `<h1>` en Fraunces y lámina decorativa, verificado leyendo el `AdminLogin.tsx` real de `atiende-restaurantes` en disco (sin tocar su `.env` ni sus datos). Las divergencias deliberadas (lámina como degradado de marca en vez de foto de cocina; tabs contraseña/enlace mágico en vez de solo Google OAuth) quedan documentadas en `apps/web/README.md` § "Paridad del login con Restaurantes" — **no** se editó `docs/investigacion/frontend-restaurantes.md` por estar fuera del ámbito exclusivo de este agente (`apps/web/**`). Reproducido/verificado con Playwright real: `docs/logs/fix-web-w11.log`, test `apps/web/e2e/login-parity.spec.ts`. |
| W-12 | Media | 2, 8 | `apps/web` no tiene ningún guard de ruta: `/panel` y todas las rutas del `AppShell` son accesibles sin sesión; el proyecto de referencia sí protege `/admin` (confirmado por render real) | Agregar un guard mínimo client-side o documentar explícitamente en el README que esta ronda no tiene control de acceso | **Corregido (documentación, opción explícitamente permitida por la reparación sugerida).** Commit `1adb386`. Se documenta en `apps/web/README.md` § "Control de acceso" por qué no se añadió un guard cosmético: no existe persistencia de sesión real todavía (`login()`/`requestMagicLink()` no guardan ningún token), así que un guard client-side solo podría simular protección sin ninguna garantía real. |
| W-13 | Media | 3, 7 | Las pruebas `vitest-axe` corren sobre el subárbol montado por Testing Library, no sobre `document` — no pueden detectar las violaciones de nivel de documento que el render real sí encontró en `/login` (W-08) | Ejecutar axe sobre `document` completo en pruebas de página, o complementar con Playwright+axe (W-14) | **Corregido (ambas mitades de la reparación sugerida).** Commit `ac6b632`. `AppShell.test.tsx`/`LoginPage.test.tsx` cambian de `axe(container)` a `axe(document.body)` — mejora real y parcial, documentada con matiz honesto en `docs/logs/fix-web-w13.log`: jsdom no reproduce el cálculo de visibilidad de `landmark-one-main`/`page-has-heading-one` ni siquiera apuntando a `document.body`, así que la verificación autoritativa de esas reglas es la suite Playwright+axe de W-14. |
| W-14 | Alta | 7, 8 | No existe ninguna suite Playwright/E2E en `apps/web`; REQ-049 y REQ-065 la exigen explícitamente | Introducir `@playwright/test` y una suite mínima sobre las pantallas existentes (puede partir de `docs/auditoria-1/capturas/capturar.mjs`) | **Corregido.** Commits `435ecbc` (infraestructura inicial) y `508c37d` (suite de recorridos completa). `apps/web/e2e/` cubre login→shell, las 24 rutas de `ALL_NAV_ITEMS`, drawer móvil a 390×844, tema oscuro, Fuentes y frescura, Paquete "Borrador" y ausencia de scroll horizontal a 390px, todas con axe-core sobre Chromium real vía `vite preview`. Script `test:e2e` (`npm run build && playwright test`) verificado en verde 44/44 dos veces seguidas y en un clon de git verdaderamente limpio (`docs/logs/fix-web-e2e.log`, `docs/logs/fix-web-ronda1.log`). |
| W-15 | Baja | 8 | REQ-115 exige disclosure de IA también en el "portal"; no existe ningún texto de ese tipo en `apps/web` | Añadir un elemento de disclosure reutilizable antes de que Redacción/Revisión muestren contenido generado | **Corregido.** Commit `345ca9f`. Se añade `<AiDisclosureNote/>` reutilizable, antepuesta al `EmptyState` de Redacción, Revisión y Análisis de bases (flag `disclosure` en `createModulePage()`). Reproducido con Playwright real antes del fix y verificado en verde después (`docs/logs/fix-web-w15.log`); test `apps/web/e2e/ai-disclosure.spec.ts`. |
| W-16 | Info | 5 | `PackageStatusBadge` acepta `"listo"` sin ninguna guarda propia; hoy no hay ningún llamador que lo produzca, pero la responsabilidad recae enteramente en el futuro integrador | Documentar (o, mejor, forzar en tipos) que `"listo"` solo puede originarse desde una validación de backend explícita | **Corregido (la opción "mejor" de la reparación sugerida).** Commit `7c37e23`. Se añade `derivarEstadoPaquete({ checklistCompleto, firmasCompletas, anexosVigentes })` como única vía sancionada para producir `"listo"` — devuelve `"borrador"` si falta cualquier condición; `PaqueteDescargablePage` la usa en vez de un literal suelto. Test `apps/web/src/components/ui/package-status-badge.test.tsx` (`docs/logs/fix-web-w16.log`). |
