...
| SR-06 | MEDIA | 2. Veracidad de integración | `parseComprasMxHistoricoCsv()` (el "bonus real y 100% verificado en vivo" del README) no está envuelto en ningún `SourceConnector` ni se invoca desde `createComprasMxConnector().discover()`; solo se ejercita desde `test/connectors/compras-mx.test.ts`. El `ConnectorRegistry`/`DiscoveryPipeline` real nunca la usa — el conector `compras-mx` registrado depende exclusivamente del endpoint bloqueado por reCAPTCHA. En la práctica, ninguna corrida real del pipeline de descubrimiento obtiene datos de ComprasMX hoy, pese a existir una función 100% verificada y probada para un dataset real. | Envolver el CSV histórico en su propio `SourceConnector` (aunque se documente como "solo benchmark histórico, no descubrimiento de convocatorias abiertas") o, como mínimo, aclarar en el README/BACKLOG que ningún dato real fluye hoy por `DiscoveryPipeline` para esta fuente. | CORREGIDO — commit `fix(sources): SR-06 ComprasMxHistoricalCsvConnector conecta el CSV histórico al pipeline`. Test que reproduce y falla antes del fix (el módulo no existía): `test/connectors/compras-mx-historical.test.ts` (6 casos: id/liveVerification propios, discover() real con el fixture recortado, límite, error explícito en status!=200, ingestión real vía `DiscoveryPipeline`, fetchDetail no implementado documentado). Fix: nuevo `createComprasMxHistoricalCsvConnector()` (`src/connectors/compras-mx/compras-mx-historical-csv-connector.ts`) envuelve `parseComprasMxHistoricoCsv()` como `SourceConnector` real, declarado explícitamente como fuente HISTÓRICA (no descubrimiento en vivo) bajo su propio `SourceId` nuevo `"compras-mx-historico"` (aditivo al enum `SourceIdSchema`, no reemplaza `"compras-mx"`). Exportado desde `src/index.ts`; `registry.test.ts` actualizado (6 conectores). Limitación documentada honestamente en el JSDoc: descarga el CSV completo en memoria (el dataset real pesa ~951 MB); streaming queda fuera de alcance de esta ronda. Suite completa: 83/83 tests en verde, typecheck y lint limpios. `apps/worker typecheck` sigue con la misma falla preexistente NO relacionada (ver nota en SR-03). |
| SR-07 | MEDIA | 4. Dedupe | `computeCrossSourceFingerprint()` (título normalizado + entidad normalizada + fecha de publicación por día) colisiona entre dos procedimientos REALES y distintos (externalId distinto) de la misma entidad, publicados el mismo día, con un título administrativo genérico compartido (confirmado con prueba propia: dos records con `externalId` "PROC-A"/"PROC-B" producen la misma huella). El riesgo práctico actual es bajo porque `findByFingerprint()` no se invoca desde `DiscoveryPipeline`/`processRecord` (no hay fusión automática hoy), pero el tipo/documentación no advierte este riesgo a un consumidor futuro. | Documentar explícitamente en el JSDoc de `findByFingerprint`/`computeCrossSourceFingerprint` que un resultado es "candidato a revisar", nunca fusión automática; considerar añadir una señal adicional (monto estimado, primeras palabras del objeto/descripción) a la huella. | CORREGIDO — commit `fix(sources): SR-07 fingerprint cruzado incluye fecha de presentación/número de procedimiento`. Test que reproduce y falla antes del fix: `test/dedupe.test.ts` > "SR-07: NO colisiona entre dos procedimientos DISTINTOS..." (dos records PROC-A/PROC-B de la misma entidad, título genérico compartido, fecha de presentación distinta). Fix: `computeCrossSourceFingerprint` (`src/dedupe/fingerprint.ts`) ahora incluye también `dateKey(dates.submissionDeadline)` y el número de procedimiento LAASSP embebido en el título si existe (`PROCEDURE_NUMBER_PATTERN`); el caso legítimo de cruce entre fuentes (mismo procedimiento, ambas fechas coinciden) se preserva (test dedicado). JSDoc de `findByFingerprint`/`computeCrossSourceFingerprint` documenta explícitamente que el resultado es "candidato a revisar", nunca fusión automática. Heurística probabilística, no garantía de unicidad (documentado). Suite completa: 86/86 tests en verde, typecheck y lint limpios. |
| SR-08 | BAJA | 8. Calidad de pruebas | No hay `vitest.config.ts` ni script con `--coverage`; no se puede cuantificar "cobertura real" más allá de conteo de tests + mutación manual (que sí se hizo, ver §8). | Agregar `@vitest/coverage-v8` y un script `test:coverage` con umbral mínimo, igual que en otros paquetes del monorepo. | CORREGIDO — commit `fix(sources): SR-08 vitest --coverage con umbrales (lineas>=85%, ramas>=80%)`. Sin test previo que "falle" (es config, no lógica): se confirmó el umbral fallando en el primer intento (branches 79.82% < 80%) y en verde tras agregar cobertura real de ramas no ejercitadas. Fix: `vitest.config.ts` nuevo con `coverage.provider: "v8"` y `thresholds.lines: 85`/`thresholds.branches: 80`; `@vitest/coverage-v8@2.1.9` agregado a devDependencies (misma major que `vitest@2.1.x`); script `test:coverage` (`vitest run --coverage`). Se agregaron tests reales para las ramas de `mapStatus`/`mapProcedureType` en `comprasmx-mapper.ts` que no tenían cobertura (aclaraciones/cerrado/fallo/cancelado/desierto/sin-estatus, tipo de contratación no catalogado, registro sin título/id). Cobertura REAL medida: 91.53% líneas, 82.33% ramas, 90.64% funciones (`npm run -w packages/sources test:coverage`, exit 0). Suite completa: 95/95 tests en verde, typecheck y lint limpios. |
| SR-09 | BAJA | 2. Veracidad de la verificación | Mi petición de solo lectura contra el endpoint real de ComprasMX (`POST .../expedientes?rows=5&page=1`, sin cabeceras reCAPTCHA) devolvió `403 {"success":false,"error":"Acceso no permitido.","details":"Acceso no permitido. - /whitney/sitiopublico/expedientes - None","pid":null}`, NO el `401 {"details":"Unauthorized"}` documentado textualmente en el README/JSDoc del conector y usado como ejemplo en `test/connectors/compras-mx.test.ts` (el test usa un mock, no depende de esto). No compromete la clasificación funcional (`classifySourceFailure` trata 401 y 403 igual: `permission_missing`), pero indica que el código/cuerpo exacto de bloqueo observado varía (probable diferencia de huella de cliente entre un WAF/edge y el backend aplicativo) y que la evidencia puntual citada como fija puede no ser estable en el tiempo. | Anotar en el README que el código de bloqueo observado puede variar entre 401 (app) y 403 (edge/WAF) según el cliente, y no depender de él para nada más que la clasificación ya robusta a ambos casos. | CORREGIDO (documental, sin cambio de comportamiento) — commit `docs(sources): SR-09 README anota variación 401/403 y nomenclatura legal vigente`. No aplica "test que falla": es una corrección de documentación, no de código (`classifySourceFailure` ya trataba 401 y 403 igual). Fix: README §ComprasMX anota explícitamente que el código/cuerpo de bloqueo observado varió entre el `401 {"details":"Unauthorized"}` documentado originalmente y el `403 {"error":"Acceso no permitido."}` que reprodujo la auditoría independiente el mismo día, sin afirmar más de lo observado (ambos = "protegido", clasificación ya robusta a ambos). De paso (instrucción del orquestador): se corrige la nomenclatura legal del portal a "Plataforma Digital de Contrataciones Públicas" (LAASSP nueva, DOF 16-abr-2025, ver `docs/legal/verificacion-legal.md`/DECISIONES D-07), aclarando que ComprasMX sigue siendo el sitio operativo vigente; el `SourceId` técnico `"compras-mx"` NO cambia (compatibilidad con `apps/worker`). Suite: 95/95 en verde (sin cambios de comportamiento). |
| SR-10 | BAJA | 8. Trazabilidad REQ | `docs/PROGRESO.md` (Ronda 1, packages/sources) lista "REQ-146..155" como alcance de la ronda, pero REQ-146 (scheduler de ingesta configurable por fuente, cadencia declarada/auditable) no tiene ninguna implementación en `packages/sources` (no hay cron/interval por fuente en `src/`). El propio README del paquete es honesto y no lo incluye en su lista de "Implementa REQ-...". `docs/BACKLOG.md` mantiene la épica E3 en "Estado: PENDIENTE", así que no hay una marca falsa de "cerrado", pero la trazabilidad podría ser más precisa. | Anotar explícitamente en `docs/PROGRESO.md` que REQ-146 (scheduler) queda pendiente de implementar, fuera del alcance de `packages/sources` (corresponde a `apps/api`/worker). | NO CORREGIDO EN ESTA RONDA (fuera de mi ámbito exclusivo) — instrucción explícita del orquestador: la trazabilidad de REQ-146 en `docs/PROGRESO.md` la corrige el propio orquestador, no este agente (mi ámbito exclusivo es `packages/sources/**`, `docs/logs/fix-sources-*.log` y esta columna de `docs/auditoria-1/sources.md`, sin tocar `docs/PROGRESO.md`). |
| SR-11 | MEDIA | 7. MatchingEngine | `MatchingEngine` solo calcula "relevancia" (score léxico 0-100); no existe ningún concepto de "elegibilidad" independiente en `src/matching/types.ts`/`matching-engine.ts`, pese a que `AMPLIACION-BACKOFFICE.md` §4 y REQ-168 (prioridad alta) exigen explícitamente exponer relevancia y elegibilidad como dos valores independientes. El README acota correctamente el alcance del paquete ("parcial, léxico") y no reclama REQ-168, pero tampoco anota este vacío en su propia lista de "Pendientes explícitos" (que sí detalla otros 5 pendientes). Para presupuesto/estado ausentes, el motor asigna un score numérico neutro (0.5) sin un campo booleano "no evaluable" distinto del score. | Agregar el punto a "Pendientes explícitos" del README de `packages/sources`, y/o exponer un campo `eligibility: "not_evaluated"` explícito en `MatchResult` para dejar sin ambigüedad que este paquete no evalúa elegibilidad. | CORREGIDO — commit `fix(sources): SR-11 MatchingEngine separa relevancia de elegibilidad (REQ-168)`. Test que reproduce y falla antes del fix: `test/matching.test.ts` (6 casos nuevos: sin criterios configurados -> "no_evaluable"; presupuesto ausente -> "no_evaluable" explícito en `eligibility` sin tocar `score`; presupuesto dentro/fuera de rango -> "cumple"/"no_cumple"; estado ausente/no-coincide -> "no_evaluable"/"no_cumple"; palabra excluida -> "no_cumple"; prioridad de "no_cumple" sobre "no_evaluable" al agregar). Fix: nuevo campo `eligibility: EligibilityResult` en `MatchResult` (`src/matching/types.ts`, aditivo: `score`/`criteria` sin cambios) con `status: "cumple"|"no_cumple"|"no_evaluable"` y `criteria[]` por requisito (`budget`/`states`/`excludedKeywords`), cada uno con su propia explicación; dato ausente nunca puntúa "cumple" en silencio. README actualizado documentando los dos valores independientes. Suite completa: 92/92 tests en verde, typecheck y lint limpios. |
