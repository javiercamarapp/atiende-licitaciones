...
| DB-09 | BAJA (nuevo) | RLS / SECURITY DEFINER (`agent_run_context`) | — | Exigir `current_user_id()` = `actor_id` o `is_superadmin()` | **PARCIAL** — `0044_fix_db09_agent_run_context_bootstrap_guard.sql` rechaza si ya hay contexto de sesión fijado (mismo patrón que `find_user_by_email`/DB-01). Gap residual documentado y probado explícitamente (`security-db09-agent-run-context.test.ts`): una llamada SIN contexto previo (la misma forma en que la invoca hoy `apps/api/src/lib/agent-stores.pg.ts`) sigue resolviendo cualquier `run_id` adivinado — cierre completo requiere pasar la identidad del actor desde ese archivo, fuera del ámbito de este agente (no es `apps/api/src/modules/auth/**` ni `organizations/**`) |
...
| API-09 | MEDIA (nuevo) | TOCTOU (refresh, tool_calls) | — | Atomizar check-y-mutación con `WHERE` de estado esperado + `rowCount` | **PARCIAL** — la mitad de refresh (`/auth/refresh`) resuelta junto con API-01 (`0043`, ver arriba). La mitad de `POST /agents/tool-calls/:id/approve|deny` (`apps/api/src/modules/agents/routes.ts`) sigue **PENDIENTE**: ese archivo no es `apps/api/src/modules/auth/**` ni `organizations/**`, fuera del ámbito de este agente |
| API-10 | MEDIA (nuevo) | Auditoría (jobs sin org) | — | Org de sistema reservada o NOT NULL relajado en `audit_log.org_id` | PENDIENTE — `apps/api/src/modules/admin/routes.ts`, fuera del ámbito de este agente |
| API-11 | MEDIA (nuevo) | Subida de documentos (REQ-024) | — | Validar magic bytes antes de `storeFile` | PENDIENTE — `apps/api/src/lib/storage.ts`, fuera del ámbito de este agente |
| API-12 | BAJA (nuevo) | Comparación no constante | — | `timingSafeEqual` en `requirePlatformApiKey` | PENDIENTE — `apps/api/src/plugins/auth.plugin.ts` no es `apps/api/src/modules/auth/**` (ese path es el módulo de rutas, no el plugin de autenticación), fuera del ámbito estricto de este agente |
