...
| DB-02 | ALTA (original) | Integridad de negocio (precio) | **REABIERTO** (parcial — ver DB-10) | Ver DB-10 | **RESUELTO** — `0042_fix_db02_db10_rate_validity_submission_deadline.sql`, vigencia validada contra `submission_deadline` (fallback a `current_date` sin plazo fijado). Test: `packages/db/test/security-db10-rate-validity-submission-deadline.test.ts` (4/4 verde) |
...
| DB-06 | MEDIA (original) | Auditoría (hash chain) | CONFIRMADO_CERRADO | Verificar `sha256()` contra versión real de Postgres | **RESUELTO (documentación)** — `0046_doc_fix_sha256_pg_version_comment.sql` corrige el comentario vía `COMMENT ON FUNCTION` (PG14+, no PG11), sin editar 0023. Sigue pendiente verificar contra la versión real de Postgres de producción antes de desplegar |
| DB-07 | BAJA (original) | Arquitectura RLS | PARCIAL (sin cambios) | — | Sin cambios, decisión de alcance vigente (fuera del ámbito de esta ronda: apps/api módulos no auth/organizations) |
| DB-08 | **CRÍTICA (nuevo)** | RLS / SECURITY DEFINER (`refresh_tokens`) | — | Validar llamador en `create_refresh_token`/eliminar o restringir `revoke_all_refresh_tokens` | **RESUELTO** — `0040_fix_db08_refresh_token_caller_context.sql`: `create_refresh_token`/`revoke_all_refresh_tokens` exigen `app.current_user_id()` coincidente (o superadmin para revoke_all) + `REVOKE EXECUTE FROM PUBLIC`. Auditoría exhaustiva de SECURITY DEFINER añadida (`security-definer-audit.test.ts`, lista blanca justificada) reveló y cerró además **DB-12** (nuevo, `app.my_organizations`, mismo patrón — `0041`). Tests: 5/5 + 8/8 (ronda2-extensions) verdes. Ataques bloqueados: acuñar refresh token para otro `user_id` → rechazado; revocar sesiones de otro usuario → rechazado; leer organizaciones de otro usuario vía `my_organizations` → rechazado |
| DB-09 | BAJA (nuevo) | RLS / SECURITY DEFINER (`agent_run_context`) | — | Exigir `current_user_id()` = `actor_id` o `is_superadmin()` | **PARCIAL, mitigado en apps/api** — `0044_fix_db09_agent_run_context_bootstrap_guard.sql` rechaza si ya hay contexto de sesión fijado (mismo patrón que `find_user_by_email`/DB-01); el gap residual (llamada SIN contexto previo, la única forma en que `apps/api/src/lib/agent-stores.pg.ts` la invoca) sigue existiendo a nivel de la función en sí. **Ronda 3 (E6-E11)**: `agent-stores.pg.ts` ahora cachea en memoria de proceso el `org_id`/`actor_id` REAL conocido desde `createRun`/`recordToolCall` (nunca del oracle) y `getRun`/`updateRun`/`listToolCalls` la consultan primero — una corrida/tool_call creada por la MISMA instancia de proceso ya NO toca la función en absoluto. El oracle solo se usa como último recurso (instancia nueva, p.ej. tras un reinicio de proceso), que es exactamente el caso residual que esta fila ya documentaba; cerrarlo por completo exige pasar la identidad del actor a través de la interfaz `RunStore`/`ToolCallStore` de `packages/agents`, fuera de este ámbito. Test que demuestra el cierre real (`security-db09-agent-run-context-cache.test.ts`, revoca `EXECUTE` sobre la función y confirma que la instancia que creó el recurso sigue funcionando mientras una instancia nueva falla) |
| DB-10 | MEDIA (nuevo) | Integridad de negocio (precio, REQ-023) | — | Validar vigencia contra `submission_deadline`, no `current_date` | **RESUELTO** — ver DB-02 arriba (mismo fix, `0042`) |
| DB-11 | BAJA (nuevo) | Documentación | — | Corregir comentario de grants de `worker_role` en 0028 | **RESUELTO (documentación)** — `0045_doc_fix_db11_worker_role_grants_comment.sql` corrige vía `COMMENT ON ROLE`, sin editar 0028 |
| DB-12 | **CRÍTICA (nuevo, no en la tabla original — encontrado durante el barrido exhaustivo de SECURITY DEFINER pedido junto con DB-08)** | RLS / SECURITY DEFINER (`my_organizations`) | — | Mismo patrón que DB-01/DB-08: exigir `current_user_id()` sin parámetro externo | **RESUELTO** — `0041_fix_db12_my_organizations_caller_context.sql`: `app.my_organizations(p_user_id uuid)` (0010, nunca tocada por 0019) permitía leer las organizaciones/roles de CUALQUIER otro usuario por id; ahora resuelve siempre sobre `app.current_user_id()`, sin parámetro. `GET /organizations` actualizado. Cubierto por `security-definer-audit.test.ts` (lista blanca + enumeración de `pg_proc`) |
| API-01 | ALTA (original) | Autenticación (refresh) | PARCIAL | Ver API-09 | **RESUELTO** — `app.rotate_refresh_token` (`0043_fix_api01_atomic_refresh_rotation.sql`) hace check-y-mutación atómicos en una sola sentencia `UPDATE ... WHERE revoked_at IS NULL RETURNING`, con detección de reuso que revoca la familia completa de sesiones activas del usuario. `apps/api/src/modules/auth/routes.ts` reescrito para usarla. Tests: `audit-api01-refresh-rotation.test.ts` (4/4 verde), incluye reuso→familia revocada, aislamiento por usuario, y doble refresh simultáneo vía `Promise.all` (exactamente 1 de 2 tiene éxito; concurrencia real de red no reproducible bajo PGlite, misma limitación documentada por la reverificación) |
| API-02 | ALTA (original) | Autorización (escalada de rol) | **REABIERTO** | Ver API-08 | **RESUELTO** — ver API-08 abajo (mismo fix) |
| API-03 | MEDIA (original) | Enumeración de usuarios | PARCIAL | Mitigar timing de `/auth/login` (delay constante o ejecutar scrypt siempre) | **RESUELTO** — `/auth/login` ahora invoca `verifyPassword` (scrypt real) siempre, con un hash ficticio de mismo formato/costo si el usuario no existe. Test `security-api03-login-timing.test.ts`: mediana 50 muestras/lado, ratio medido 1.03x-1.14x en corridas repetidas (umbral de aceptación < 1.5x, holgado frente al 24x medido antes del fix) |
...
| API-08 | **ALTA (nuevo)** | Autorización (invitación como owner) | — | Replicar el chequeo del PATCH en `POST /organizations/invitations` | **RESUELTO** — regla `role==='owner' && request.orgRole!=='owner' → 403` replicada en `POST /organizations/invitations` (`apps/api/src/modules/organizations/routes.ts`). Test `security-api08-invite-owner-escalation.test.ts` (4/4 verde). Ataques bloqueados: admin invita con `role:'owner'` → 403, 0 invitaciones persistidas; admin invita hasta `admin` → sigue permitido |
| API-09 | MEDIA (nuevo) | TOCTOU (refresh, tool_calls) | — | Atomizar check-y-mutación con `WHERE` de estado esperado + `rowCount` | **RESUELTO** — la mitad de refresh (`/auth/refresh`) resuelta junto con API-01 (`0043`, ver arriba). **Ronda 3 (E6-E11)**: la mitad de `POST /agents/tool-calls/:id/approve|deny` (`apps/api/src/modules/agents/routes.ts`) cerrada — el `UPDATE` ahora trae `AND authorization_status = 'pending'` en su propio `WHERE` (antes el único guard era un `SELECT` previo); solo diagnostica `not_found` vs. `not_pending` con una consulta posterior si `rowCount=0`. Test `security-api09-tool-calls-atomic.test.ts`: dos aprobaciones concurrentes (`Promise.all`) siempre exactamente un 200 y un 409; honesto sobre que bajo PGlite (conexión física única) el test ya pasaba con el código anterior, igual que constató esta misma reverificación — el cierre es estructural (misma sentencia atómica que ya usa API-01), no una transición roja→verde observable en este entorno |
| API-10 | MEDIA (nuevo) | Auditoría (jobs sin org) | — | Org de sistema reservada o NOT NULL relajado en `audit_log.org_id` | **RESUELTO (ronda 3, E6-E11)** — `0035_fix_api10_audit_log_nullable_org.sql` relaja `audit_log.org_id` a NULL-able (la política RLS de INSERT ya lo permitía para `is_superadmin()`; `app.audit_log_row_payload`, migración 0023, ya usaba `coalesce(org_id, '')` anticipando este caso). `apps/api/src/modules/admin/routes.ts` elimina los tres `if (org_id)` que omitían la auditoría (`jobs/:id/retry`, `incidents` create/resolve) y audita siempre. Test `security-api10-audit-log-null-org.test.ts`: job/incidente sin organización reintentado/creado/resuelto por un superadmin queda en `audit_log` con `org_id = null` |
| API-11 | MEDIA (nuevo) | Subida de documentos (REQ-024) | — | Validar magic bytes antes de `storeFile` | **RESUELTO (ronda 3, E6-E11)** — `apps/api/src/lib/storage.ts`: `storeFile` corre `assertSafeFileContent` antes de escribir a disco, rechazando (422) firmas de ejecutables (PE/ELF/Mach-O/class de Java) y de llaves privadas/certificados en bruto (PEM o prefijo ASN.1 DER `0x30 0x82`), independiente del `documentType` declarado (lista negra mínima fail-closed, no una lista blanca completa — el esquema no declara qué extensión se espera por categoría). Test `security-api11-file-magic-bytes.test.ts`: ejecutable/PEM/DER rechazados con 422 sin escribir nada a disco; documento de texto legítimo sigue aceptándose (sin falsos positivos) |
| API-12 | BAJA (nuevo) | Comparación no constante | — | `timingSafeEqual` en `requirePlatformApiKey` | **RESUELTO (ronda 3, E6-E11)** — `apps/api/src/plugins/auth.plugin.ts`: `requirePlatformApiKey` ahora usa `timingSafeEqual` (mismo patrón que `lib/passwords.ts`), igualando longitudes explícitamente primero (`timingSafeEqual` lanza si los buffers difieren de tamaño). Test `security-api12-platform-key-constant-time.test.ts`: clave correcta autentica; variantes más corta/más larga/distinta en el primer o último byte se rechazan igual (401), sin regresión funcional |
