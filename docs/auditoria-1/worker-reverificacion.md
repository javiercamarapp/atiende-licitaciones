...
> "Estado reparación" se agregó en esta ronda (corrector, vuelta 2) para dar
> seguimiento a los 5 hallazgos nuevos, siguiendo el mismo formato que
> `docs/auditoria-1/agents-reverificacion.md`. El resto del texto de esta
> tabla (columnas "Hallazgo"/"Evidencia") es el original de la
> reverificación, sin alterar. Salida real de
> `typecheck`/`lint`/`test`/`test:coverage` tras esta ronda en
> `docs/logs/fix-worker-ronda3.log`.

| ID | Severidad | Rubro | Hallazgo | Evidencia | Estado reparación |
|---|---|---|---|---|---|
| WK-14 | **ALTA** | 2. Cola / fencing | `complete()`/`fail()`/`deadLetterPermanent()` no verifican `expectedAttempts` (a diferencia de `heartbeat()`), solo `locked_by`. Bajo reutilización del mismo `WORKER_ID` tras un reinicio, un zombie de una generación anterior puede terminar el job de la generación activa. | Test propio §5 del log: `complete(jobId,'worker-A')` de la generación 1 marca `succeeded` un job que la generación 2 (mismo `workerId`, `attempts` distinto) sigue "corriendo". | **Corregido** (commit `ab5fa95`) — `claim()` (`apps/worker/src/queue/job-queue.ts`) genera ahora un lease token (UUID) NUEVO en cada reclamo (inicial o por recuperación de lease), embebido en `locked_by` como `${workerId}::${uuid}`, sin migración de esquema nueva. `heartbeat()`/`complete()`/`fail()`/`deadLetterPermanent()` exigen ese `locked_by` EXACTO (no solo el prefijo `workerId`) y las 3 últimas lanzan `StaleLeaseError` — sin persistir ningún cambio — si no coincide; `Worker.process()` captura `job.lockedBy` en el claim original y maneja `StaleLeaseError` explícitamente (métrica `fenced`, nunca reintenta con un lease ya inválido). Tests nuevos en `apps/worker/test/job-queue.test.ts` describe "WK-14": WORKER_ID reutilizado tras "reinicio" (mismo string, lease token distinto) → `complete()`/`fail()`/`deadLetterPermanent()`/`heartbeat()` del zombie de la generación anterior, todos rechazados; el job queda intacto, propiedad de la generación nueva. |
| WK-16 | **ALTA** | 6. run_agent / 7. Seguridad | `organizationId: null` en el payload de `run_agent` desactiva por completo la verificación cruzada de tenant de WK-08 (`$5::uuid is null` hace que el `WHERE` se reduzca a `id=$1`). | Test propio §8 del log: `agentRunId` real de orgA + `organizationId: null` → `UPDATE` exitoso sin error, fila de orgA sobrescrita. | **Corregido** (commit `95f3a03`) — `createRunAgentHandler()` (`apps/worker/src/handlers/run-agent.ts`) ahora falla fail-closed ANTES de correr el `AgentRunner` o tocar `agent_runs`: si `job.payload.agentRunId` está presente y `organizationId` es `null`/`undefined`/`''` (los 3 valores "sin org"), lanza `RunAgentMissingOrganizationError` (permanente, WK-10) con mensaje "org requerida". `updateAgentRunRow` ya no acepta `organizationId` nulo y el `UPDATE` compara siempre `org_id = $5::uuid` (sin el escape `is null` de antes). Tests nuevos parametrizados en `apps/worker/test/run-agent-handler.test.ts` describe "WK-16": `null`/`undefined`/`''` con `agentRunId` presente → falla permanente, la fila real de `agent_runs` nunca se toca (`status` sigue `running`, `output` sigue `null`). |
| WK-15 | MEDIA | 4. Scheduler / 8. Calidad de pruebas | El test oficial de WK-04 no demuestra protección de concurrencia real: PGlite serializa transacciones completas sin intercalar, así que el mismo test pasaría igual sin el advisory lock. | Diagnóstico propio §7b del log: orden de ejecución observado sin intercalado alguno entre dos `transaction()` concurrentes. | **Corregido (documentación + test adicional)** (commit `b6b879f`) — se reescribe la afirmación en `apps/worker/README.md` y en el docstring del test oficial de WK-04 (`apps/worker/test/scheduler.test.ts`): "el SQL (`pg_advisory_xact_lock`) es correcto para Postgres real; en PGlite se verifica solo la lógica secuencial", y se marca la verificación de concurrencia de motor real como **PENDIENTE (ref. B-03, requiere Postgres de pruebas en CI)** tanto en el README (§Pendientes) como en el docstring del test. Test nuevo describe "WK-15 — el SQL emitido usa pg_advisory_xact_lock con una clave estable" (`apps/worker/test/scheduler.test.ts`): intercepta (sin mocks, contra PGlite real) las sentencias ejecutadas dentro de la transacción de `enqueue()` y confirma que la primera es textualmente `pg_advisory_xact_lock(hashtext($1)::bigint)` con la clave estable `${kind}:${jobKey}` como parámetro, estable entre llamadas repetidas y distinta entre `jobKey`s distintos. |
| WK-17 | MEDIA | 5. Honestidad ingesta | HTTP 408 se clasifica como error PERMANENTE (dead-letter inmediato) en vez de transitorio, contradiciendo la semántica HTTP estándar y el criterio pedido explícitamente para esta reverificación. | Test propio §10 del log contra servidor HTTP real: `server.hits===1` (nunca reintentado), `.permanent===true`, `isPermanentJobError===true`. | **Corregido** (commit `e9879f3`, fix de typecheck en `521b321`) — `RETRYABLE_STATUS` (`apps/worker/src/ingest/ingest-client.ts`) ahora incluye `408` y `425` (Too Early, mismo problema, nunca antes verificado explícitamente) además de los ya presentes `429`/`500`/`502`/`503`/`504`. Test nuevo describe "WK-17: tabla de verdad — transitorio vs. permanente por código HTTP" (`apps/worker/test/ingest-client.test.ts`), contra un servidor HTTP real (no un mock de `fetch`): `408`/`425`/`429`/`500`/`502`/`503`/`504` → transitorio (reintenta hasta `maxRetries`); `400`/`401`/`403`/`404`/`422` → permanente (0 reintentos); error de red sin status → transitorio. |
| WK-18 | BAJA | Trazabilidad / gobierno | El commit citado para WK-13 en la tabla de `docs/auditoria-1/worker.md` no existe; la corrección real está en el commit `3bc21f6` ("docs(sources): SR-09..."), sin relación aparente con WK-13 en su mensaje — probable artefacto de una carrera de commits concurrentes. | `git log -p -- apps/worker/src/handlers/run-agent.ts`: el bloque `declaredEffects` aparece introducido en `3bc21f6`, no en un commit "fix(worker): WK-13...". | **Documentado, sin cambio de código** — el commit real (`3bc21f6`, "docs(sources): SR-09 README anota variación 401/403 y nomenclatura legal vigente") ya existe y ya está en el historial de `main`; no hay nada que reparar en el código (`declaredEffects` sigue correcto en HEAD, confirmado por `typecheck`/`test` en verde). Esta fila documenta, de forma permanente, la referencia correcta al commit real para que quien consulte `docs/auditoria-1/worker.md` no busque un commit "fix(worker): WK-13..." que no existe. |

**Cierre de PARCIAL WK-02/04/08/10**: los veredictos "PARCIAL" de WK-02,
WK-04, WK-08 y WK-10 en la tabla de la §2 (arriba) quedaban así
específicamente por los gaps documentados en WK-14, WK-15, WK-16 y WK-17
respectivamente. Con esos 4 hallazgos nuevos corregidos (ver columna
"Estado reparación" de esta tabla) y sus tests en verde dentro de la misma
suite de `apps/worker` (79 tests, ver `docs/logs/fix-worker-ronda3.log`), los
4 huecos quedan cerrados; el texto original de los veredictos PARCIAL de la
§2/§3 no se modifica aquí (fuera del ámbito de esta columna), pero su cierre
queda documentado en esta nota y en el log de esta ronda.
