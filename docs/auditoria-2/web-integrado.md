...
|---|---|---|---|---|
| WI-01 | Media | 6 (Seguridad frontend) | Cero Content-Security-Policy en todo el sistema (`contentSecurityPolicy: false` en `apps/api`, y `apps/web` servido sin ninguna cabecera de seguridad propia) — agrava el riesgo ya documentado de refresh token legible por XSS en `localStorage` | **RESUELTO (lado apps/api)** — `contentSecurityPolicy` real (`default-src`/`script-src 'self'`; `connect-src 'self'` + `CORS_ORIGINS`; `frame-ancestors 'none'`; `object-src 'none'`) + `Permissions-Policy` explícito en toda respuesta (incluidas 4xx/5xx y `/docs/json`). Test: `apps/api/test/security-wi01-csp-headers.test.ts`. La mitad de `apps/web` (cabeceras propias del sitio servido) sigue pendiente, fuera de este ámbito. |
| WI-02 | Media | 6 (Seguridad frontend) / REQ-098 | `<input type="file">` de subida de documentos sin `accept` ni validación de tipo/tamaño en cliente; el servidor sí valida bien (magic bytes + 22MB), pero el criterio explícito de REQ-098 pide rechazo también en UI | Fuera de este ámbito (`apps/web`). |
| WI-03 | Baja/Media | 2 (Sesión) | `logout()` no limpia el caché de react-query (`queryClient.clear()` ausente); sin fuga entre organizaciones (las queries de negocio SÍ incluyen `currentOrgId` en su clave), pero sí un residuo visible de sesión anterior en las queries admin/globales tras un logout real en navegador compartido | Fuera de este ámbito (`apps/web`) — resuelto por el agente de `apps/web` (commit `74c2358`, ver historial de ese workspace). |
| WI-04 | Baja | 2/3 (Idempotencia / Permisos) | Botones "Aprobar"/"Rechazar" de tarifas sin `disabled` durante `isPending` (a diferencia de "Proponer"); compuesto por que la API tampoco guarda el estado previo al actualizar (`UPDATE ... SET status = ...` sin condición de estado), permitiendo re-aprobar/re-rechazar y generando entradas de auditoría redundantes para el mismo evento | **RESUELTO (lado apps/api)** — `POST /company/rates/:id/approve\|reject` ahora son transiciones atómicas (`UPDATE ... WHERE ... AND status='draft' RETURNING`, mismo patrón que API-09 en `tool_calls`): re-aprobar/re-rechazar una tarifa ya decidida responde 409 explícito, un solo evento de `audit_log` por decisión, dos aprobaciones concurrentes nunca ambas 200. Test: `apps/api/test/security-wi04-approved-rates-atomic.test.ts`. La mitad de `apps/web` (`disabled` durante `isPending`) sigue pendiente, fuera de este ámbito. |
| WI-05 | Baja | 1 (Reproducibilidad) | `test:e2e:full` no es determinísticamente verde por un límite de tasa real de `apps/api` (100/min) autoinducido por el barrido de 29 rutas de `skip-link.spec.ts`; sin `RATE_LIMIT_PROFILE` ni variable equivalente en el código para mitigarlo en modo test | **DEPENDENCIA RESUELTA (lado apps/api)** — `RATE_LIMIT_PROFILE=e2e` ya existe (ver `apps/api/README.md`/`.env.example`, `lib/rate-limit-settings.ts`): eleva el límite global a 3000/min y el de `/auth/login` a 300/min, activo SOLO con esa variable exacta. Falta que `apps/web` (`scripts/e2e-full.mjs`) la defina al levantar `apps/api` para la suite E2E -- fuera de este ámbito. |
