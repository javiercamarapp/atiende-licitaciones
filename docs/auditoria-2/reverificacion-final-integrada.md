...
| **WI-06** (nuevo) | BAJA | **NO CERRADO** (nunca reparado) | Doble clic físico real en "Aprobar" dispara 2 peticiones (el guard cliente no gana la carrera); servidor sigue siendo la barrera real, sin duplicar efecto ni auditoría | **REPARADO** — guard síncrono (`useRef` fijado ANTES de `mutate()`, independiente del ciclo de render) en `TarifasAprobadasPage.tsx`; E2E con doble clic físico real (`page.mouse.click` en las mismas coordenadas, sin esperar entre clics) confirma 1 sola petición de red y ningún toast de error (`apps/web/e2e/ronda3-flujo-real.spec.ts`). Ver `docs/logs/fix-api15-wi06.log` |
