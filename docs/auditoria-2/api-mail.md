...
| Id | Severidad | Rubro | Resumen | Estado reparación |
|---|---|---|---|---|
| AM-01 | ALTA (límite CRÍTICA) | 5 | Vincular Google a una cuenta email+contraseña no verificada no revisa el estado ni la contraseña preexistente → toma de cuenta si la verificación nativa se completa después | **Reparado** (commit `b70a1ea`): `google/routes.ts` invalida `password_hash`/revoca sesiones y marca `email_verified_at` en el mismo acto de vincular si la cuenta estaba squatteada. Test: `security-am01-google-account-takeover.test.ts`. |
| AM-02 | ALTA | 3 | `/auth/password/forgot` y `/auth/email/resend-verification` reabren el oráculo de temporización que API-03 cerró para `/auth/login` (8.34x / 5.83x) | **Reparado** (commit `640ecce`): `fireAndForgetMail` se dispara SIEMPRE (real o decoy vía `lib/mail/decoy.ts`). Test: `security-am02-mail-timing.test.ts` (ratios 1.06-1.19x, umbral 1.5x). |
| AM-03 | ALTA | 7/9 | El webhook suprime cualquier dirección de un payload con firma válida sin verificar que corresponda a un envío nuestro real → puede silenciar correo de seguridad de un usuario real | **Reparado** (ver commit de esta ronda): `modules/mail/webhook.routes.ts` exige, antes de suprimir por bounce/queja, que `providerMessageId` corresponda a una fila real de `mail_outbox` CON el mismo destinatario (`lib/mail/pg-outbox-lookup.ts`); un evento incoherente se acepta (202) pero se ignora, sin efecto de negocio, y queda auditado (`audit_log.entity = 'mail_webhook'`). Requirió además respaldar `mail_outbox.to_email` (nunca escrito hasta ahora) desde `lib/mail/send-transactional.ts`. Nota de alcance: la lectura/escritura contra `mail_outbox` usa `db.query` directo (sin `set local role app_role`) porque la función SECURITY DEFINER equivalente (`app.mail_outbox_find_by_provider_message_id`) requeriría una migración de `packages/db`, fuera del ámbito asignado a este agente — documentado en el propio código para que una ronda con `packages/db` en su ámbito lo reemplace. Test: `mail-webhook.test.ts` (bloque "AM-03"). |
| AM-04 | MEDIA | 6 | `app.enqueue_mail_retry` no valida `org_id` contra la sesión invocadora; payload sin validación de forma más allá de `messageKey` | Reparado fuera de esta ronda (`packages/db/migrations/0089_am04_enqueue_mail_retry_org_check.sql`) — no tocado por este agente (fuera de ámbito). |
| AM-05 | MEDIA | 11 | `correlation_id` nunca se propaga en la familia `auth.*`/correo; el envío inicial de verificación no se audita como acción propia | **Reparado** (commit `c98bb30`, ronda previa): `recordAuthAudit`/`app.record_auth_event` (0096) propagan `correlationId` en las 16 llamadas de la familia `auth.*`. Esta ronda añade la prueba que faltaba: `security-am05-correlation-id-auth-family.test.ts` (las 4 acciones de correo, sesiones, cambio de contraseña, y una comprobación genérica que falla si CUALQUIER evento `auth.*` queda con `correlation_id` NULL). Pendiente, sin cerrar en esta ronda (no forma parte del hallazgo de `correlation_id`): el envío INICIAL de verificación en `POST /auth/register` sigue sin su propio evento de auditoría. |
| AM-06 | BAJA | 12 | `docs/ACEPTACION.md` sigue marcando S4-S12/REQ-181..210 como "Sin código/PENDIENTE" pese a tener código y pruebas en verde | Sin tocar en esta ronda (fuera del ámbito asignado: `apps/api/src/modules/auth/**`, `lib/mail/**`, `modules/mail/**`). |
