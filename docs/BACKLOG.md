...
**Estado: EN CURSO** — depende de E7 (cerrado). El ciclo de aprobación/invalidación/re-derivación de estado está cerrado y reverificado (A11/A14 CUMPLE). **Ronda 5 cierra REQ-044/064**: 2FA TOTP real con step-up de un solo uso atado a `(usuario, org, propósito)`, límite de tasa + bloqueo progresivo + auditoría de fallos con IP, extendido a la aprobación de `tool_calls` (org-scoped y cross-org de superadmin, R5-11) y conectado en `apps/web` con diálogos de step-up reales (RF-01) — cadena completa auditoría→corrección→reverificación independiente en `docs/auditoria-2/api-ronda5.md`, `api-ronda5-reverificacion.md`, `api-r5-09-10-reverificacion.md`, `ronda5-final.md`, `ronda5-final-reverificacion.md`. **2026-09-10 cierra REQ-040** (checklist de "sala de guerra"): `WarRoomChecklist` real en `packages/expediente` + persistencia append-only (`war_room_checklist_runs`, migración 0099) + rutas en `apps/api` + página en `apps/web` (`/entrega/sala-de-guerra`) — ver `docs/ACEPTACION.md` para el detalle y la evidencia de pruebas (30/30 en verde entre unit/integración/RLS). Sigue en curso porque passkey/WebAuthn (matiz de REQ-044/064, solo TOTP hoy) y el juez LLM distinto del redactor (REQ-039/127, bloqueado por OpenAI) no están construidos.
...
- **E8**: implementar passkey/WebAuthn como segundo factor adicional al TOTP ya cerrado (matiz de REQ-044/064); construir el juez LLM distinto del redactor (bloqueado por falta de credenciales OpenAI). La "sala de guerra" (REQ-040) ya está construida (2026-09-10) — pendiente solo una auditoría/reverificación adversarial independiente para pasar de EN_EVIDENCIA a CUMPLIDO.
