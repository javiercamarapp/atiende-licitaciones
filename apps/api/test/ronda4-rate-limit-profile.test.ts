...
import { createTestApp, TEST_JWT_SECRET, TEST_TOTP_ENCRYPTION_KEY, TEST_MAIL_LINK_SECRET } from './helpers.js';
  ...
  it('loadConfig() sin RATE_LIMIT_PROFILE resuelve "default", incluso con NODE_ENV=test/development', () => {
    for (const nodeEnv of ['test', 'development', 'production', undefined]) {
      const cfg = loadConfig({
        ...
        // REQ-181..195: `loadConfig` ahora exige MAIL_LINK_SECRET (>=16
        // caracteres) igual que JWT_SECRET/TOTP_ENCRYPTION_KEY -- este test
        // solo mira `rateLimitProfile`, pero la config es una sola.
