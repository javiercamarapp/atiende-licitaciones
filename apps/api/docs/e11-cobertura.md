# E11 — Cobertura real contra REQ-050..056 (ronda 5, actualizado ronda 6, ronda 7 y ronda 8)
...
| REQ-053 | Redactor de inconformidades: borrador estructurado (hechos/agravios/fundamentos con jurisdicción y fecha DOF/pruebas/plazo), guardrail anti-frivolidad, versionado con hash, step-up para "marcar como revisado" | `lib/expediente/inconformidad.ts` (`buildInconformidadContent`); `lib/expediente/business-days.ts` (`computeInconformidadDeadline`, Art. 95 LAASSP); `modules/expediente/inconformidad.routes.ts`; tabla `inconformidad_drafts` (migración 0068) | `apps/api/test/expediente-inconformidad.test.ts` | **CONSTRUIDO (ronda 6)** — cada generación crea una VERSIÓN nueva (nunca edita una existente; trigger de base de datos hace el contenido INMUTABLE incluso para el dueño), con `contentHash` (sha256). Fundamentos citan LAASSP nueva Art. 49 (fallo, motivación del desechamiento) y Art. 95 (plazo: 6 días hábiles, o 10 bajo cobertura de tratados), ambos con jurisdicción "Federal" y fecha DOF 2025-04-16 (`docs/legal/verificacion-legal.md`, filas REQ-103/REQ-104). El plazo se calcula con el MISMO motor determinista de días hábiles que REQ-050 (nunca un LLM), combinando el calendario oficial (`calendar_holidays`) si está cargado. Todo borrador queda marcado con el disclaimer "BORRADOR — requiere revisión de abogado" y JAMÁS se envía a ninguna autoridad (sin cliente HTTP saliente en el módulo). Guardrail anti-frivolidad determinista (pruebas vs. agravios) clasifica `viability` (`alta`/`media`/`baja`) con una recomendación explícita — NUNCA bloquea la generación, solo advierte. "Marcar como revisado" exige step-up (2FA, `purpose='expediente.inconformidad_review'`) y rol reviewer/admin/owner; ya revisado → 409. **Ronda 7** añade `sourceAutopsyId` opcional: vincula el borrador a una autopsia del fallo (REQ-054) ya registrada y anexa a `hechos` un restatement puramente factual de lo YA capturado ahí (motivo declarado, precio propio vs. ganador, comparación de criterios) — nunca reemplaza `hechos` capturados a mano, solo los complementa; rechaza (422) vincular una autopsia con `ownProposalStatus='ganadora'`, y exige al menos un hecho (manual o derivado) antes de generar. Los `agravios` siguen siendo SIEMPRE responsabilidad de un humano — nunca se derivan de la matriz/autopsia (ver nota E9 en el código: un hueco en el checklist propio no es automáticamente una irregularidad de la convocante). Ver `apps/api/test/expediente-inconformidad.test.ts` (casos ronda 7). |
| REQ-054 | Autopsia del fallo: comparación estructurada propuesta propia vs. fallo (motivo de desechamiento, puntos/criterios, precio vs. ganador), lecciones vinculadas al perfil de empresa, sin inventar datos ausentes | `modules/expediente/fallo-autopsy.routes.ts`; tablas `fallo_autopsies`/`company_lessons_learned` (migración 0069) | `apps/api/test/expediente-fallo-autopsy.test.ts` | **CONSTRUIDO (ronda 6)** — cualquier campo textual no capturado (motivo de desechamiento, nombre del ganador) se persiste literalmente como `"no disponible"` (constante `NO_DISPONIBLE`), nunca NULL/vacío ambiguo ni inventado; precio/puntos del ganador quedan `null` explícito si el fallo no es público. Comparación de criterios/puntos como lista estructurada libre (`criteriaComparison`). Lecciones registradas en `company_lessons_learned`, vinculadas a la autopsia Y consultables ORG-WIDE (no solo por convocatoria) vía `GET /expediente/lessons-learned` — "vinculadas al perfil de empresa" tal como pide el requisito. No se construyó una taxonomía CERRADA de motivo de pérdida (el texto original de REQ-054 en este documento la menciona); `disqualificationReason` es texto libre capturado por el usuario, límite documentado. **Ronda 7** añade `GET .../fallo-autopsy/analysis` (`lib/expediente/fallo-analysis.ts`, `analyzeFalloCauses`, función pura): compara la autopsia registrada contra la matriz de requisitos obligatorios de la convocatoria (E6, `requirement_items.matrix_status` — el campo de gestión real que el equipo edita vía `PATCH /matrix/:id`; `compliance_items.requirement_id` no se usó porque ningún endpoint lo escribe hoy, sería una señal fingida) y deriva "posibles causas de no adjudicación" (`fallo_declarado` = lo ya declarado en el acta; `requisito_pendiente`/`requisito_bloqueado` = requisitos obligatorios sin `matrix_status='cumplido'`). SIEMPRE devuelve `disclaimer` ("ANÁLISIS AUTOMATIZADO... NO es un diagnóstico certero...") y, cuando falta la autopsia, la matriz, o el motivo de desechamiento real, lo declara en `missingDataNotes` en vez de inventar una causa — nunca genera `possibleCauses` si `ownProposalStatus='ganadora'` o si el dato base no existe. Es de solo lectura (no persiste un reporte, se recalcula en cada llamada sobre el estado vigente de la matriz). Ver `apps/api/test/expediente-fallo-autopsy.test.ts` (describe ronda 7). |
| REQ-055 | Radar de renovaciones: a partir de contratos con fecha de fin y convocatorias históricas de la misma entidad, alertas de renovación probable con antelación configurable; jobs en tabla `jobs`, sin envío externo | `lib/expediente/renewal-radar.ts` (`computeRenewalAlertCandidates`, pura; ronda 8: `computeUpcomingRenewals`/`urgencyForLeadDays`); `modules/expediente/renewal-radar.routes.ts` (`POST /renewals/scan`, `GET /renewals/alerts`; ronda 8: `GET /renewals/upcoming`); columnas `contracts.end_date`/`contract_number` (ronda 8: `has_renewal_option`/`renewal_option_notes`) + tabla `renewal_alerts` (migración 0070, migración 0095) | `apps/api/test/expediente-renewal-radar.test.ts` | **CONSTRUIDO (ronda 6, ronda 8 añade el "cliente concreto")** — escaneo BAJO DEMANDA (sin cron real, ver README) sobre los contratos de la organización con `end_date` conocida; umbrales de antelación configurables (por defecto 90/60/30 días); cada umbral cruzado genera una alerta (`renewal_alerts`) + un job `renewal_radar_alert` (`jobs`, sin envío externo), con índice único parcial que evita duplicar el mismo umbral en escaneos repetidos. Cada alerta se enriquece con hasta 5 convocatorias PREVIAS de la MISMA organización y el MISMO `contracting_body` (si existen) como contexto de apoyo. **Ronda 8** construye el cliente concreto que el backlog señalaba como faltante (el mecanismo genérico de alertas post-adjudicación, `kind='hito'`, es reutilizable pero no es esto): migración 0095 añade `contracts.has_renewal_option`/`renewal_option_notes` (metadata declarativa, capturada por el usuario vía `PATCH .../contract`, nunca inferida) para distinguir un contrato con opción de renovación pactada de uno que simplemente termina; `GET /renewals/upcoming` (nuevo, solo lectura, no requiere haber corrido `/renewals/scan` antes) calcula en vivo los tres umbrales 90/60/30 (o los que se pidan vía `?thresholds=`) de forma SIMULTÁNEA y EXPLÍCITA — semántica "todos los que aplican": un contrato a 20 días de vencer aparece en los tres grupos a la vez, nunca colapsado al más urgente — y los agrupa por `urgency` (`urgente`/`proxima`/`seguimiento`) para consumo directo de un cliente de negocio. Fechas límite exactas (90/60/30 días, un día antes/después de cada umbral) verificadas en `test/expediente-renewal-radar.test.ts`. LÍMITE DOCUMENTADO: no se implementó predicción de una licitación futura a partir SOLO de convocatorias históricas cuando no existe todavía un contrato propio con `end_date` (p. ej. una dependencia que nunca adjudicó antes a esta organización) — `sourceKind='historical_pattern'` está declarado en el esquema pero no se genera en esta ronda. |
...
## Ronda 7 — REQ-053/054, análisis automatizado + vínculo autopsia→inconformidad

Encargo despachado explícitamente sobre lo que ronda 6 ya había construido
(el endpoint manual de `fallo_autopsies` y el redactor de inconformidades):
que la autopsia del fallo compare la propuesta propia contra la matriz de
requisitos conocida en vez de depender solo de campos capturados a mano, y
que el redactor de inconformidades pueda partir de una autopsia ya
registrada. Nada de lo de ronda 6 se modificó ni se rompió (los 11 tests
existentes de REQ-053/054 siguen pasando sin cambios); esto es una capa
ADITIVA.

- **`lib/expediente/fallo-analysis.ts`** (nuevo, función pura sin acceso a
  base de datos ni red, mismo patrón que `business-days.ts`/`inconformidad.ts`):
  `analyzeFalloCauses({ autopsy, requirements })` deriva "posibles causas de
  no adjudicación" combinando lo que el usuario ya declaró en la autopsia
  con los requisitos OBLIGATORIOS de la matriz (E6) que no llegaron a
  `matrix_status='cumplido'`. Nunca determina la causa real (solo la
  convocante la sabe); cada hueco de información (sin autopsia, sin matriz,
  sin motivo de desechamiento capturado, propuesta ganadora) se declara en
  `missingDataNotes` en vez de rellenarse con una suposición. `disclaimer`
  fijo, siempre presente, deja explícito que es un análisis automatizado
  sujeto a revisión humana, no un diagnóstico certero.
- **`GET /expediente/tenders/:tenderId/fallo-autopsy/analysis`** (nuevo,
  solo lectura, cualquier rol autenticado de la organización): carga la
  autopsia más reciente + los ítems activos de `requirement_items` y llama
  a `analyzeFalloCauses`. No persiste nada — se recalcula sobre el estado
  vigente de la matriz en cada llamada, así que si el equipo actualiza
  `matrix_status` después, el análisis lo refleja sin necesidad de
  regenerar un reporte guardado.
- **Decisión de diseño documentada**: se usó `requirement_items.matrix_status`
  (el campo de gestión que el equipo edita en vivo vía `PATCH
  /tenders/:tenderId/matrix/:id`) como señal de "¿esto ya se cumplió?", NO
  `compliance_items.requirement_id`/`.status` (que existe en el esquema
  desde ronda 1 pero ningún endpoint de `apps/api` lo escribe hoy — el
  checklist real que sí está conectado es el de 7 dimensiones por
  `dimension`, no por requisito individual). Usar una columna sin escritor
  real habría fingido una señal de cumplimiento que nadie llena.
- **`sourceAutopsyId` opcional en `POST /expediente/tenders/:tenderId/inconformidad`**
  (`inconformidad.routes.ts`, `buildHechosFromAutopsyRow`): vincula el
  borrador a una autopsia ya registrada y anexa a `hechos` un restatement
  puramente factual de lo que ESA autopsia ya tiene capturado (motivo de
  desechamiento o su ausencia explícita, precio propio vs. ganador,
  comparación de criterios) — nunca inventa nada nuevo, cada hecho generado
  queda atribuido ("según lo capturado en la autopsia"). Rechaza (422)
  vincular una autopsia con `ownProposalStatus='ganadora'` y exige al menos
  un hecho (manual o derivado) antes de generar cualquier borrador.
  **Decisión legal deliberada**: los `agravios` (fundamento de la
  impugnación) NUNCA se derivan automáticamente de la matriz/autopsia —
  un requisito de la matriz propia sin cumplir es responsabilidad del
  cliente, no necesariamente una irregularidad de la convocante, y
  fabricar un "agravio" a partir de un hueco propio sería jurídicamente
  irresponsable (E9, "nunca afirmar certeza que no existe"); ese campo
  sigue siendo 100% redactado por un humano.
- **Tests**: `describe` nuevo en `expediente-fallo-autopsy.test.ts` (4 casos:
  sin datos → honesto; matriz mixta → causas correctas, ignora
  cumplido/opcional; ganadora → no aplica; motivo no capturado → sin causa
  fabricada) y 4 casos nuevos en `expediente-inconformidad.test.ts`
  (sin hechos ni autopsia → 422; con autopsia → hechos derivados +
  agravios intactos; autopsia ganadora → 422; autopsia inexistente → 404).
  19/19 tests de ambos archivos en verde; suite completa de `apps/api`
  verificada sin regresiones. `npm run typecheck`/`lint`/`test --workspace=apps/api`
  en verde. Sin migraciones nuevas (reutiliza tablas existentes).
